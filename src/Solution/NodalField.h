...
//****************************************************************************80
  ...
  {
    return MeshField::data_->operator()(n,j);
  }
//****************************************************************************80
...
//****************************************************************************80
  ...
  for(intT i = 0; i < nvar_(n); i++){
      data(i) = MeshField::data_->operator()(n,i);
    }
  }
...
//****************************************************************************80
  ...
  {
    return MeshField::data_->get_ptr(n,0);
  }
//****************************************************************************80
  ...
  inline void ElementData(const intT& e, Array1D<realT>& data)
  {
    ...
    for(intT i = 0; i < ndof; i++){
      ...
      for(intT j = 0; j < nvar_(dof); j++){
        data(k) = MeshField::data_->operator()(dof,j);
        k++;
      }
...
//        data(i,j) = MeshField::data_->operator()(data_index_(node) + j);
//      }
//
//    }
//
...
private:
  const List2D<intT>& elem2dof_;//!< Lists the dof's the belong to an element
  const Array1D<intT>& nvar_;//!< Number of variables per node
