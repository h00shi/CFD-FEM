//-*-c++-*-
#ifndef STATICLIST2D_H
#define STATICLIST2D_H

//****************************************************************************80
//! \class StaticList2D
//! \brief StaticList2D : A 2-D ordered list class, static data version.  
//!                   Think of it like 
//!                  can non-uniform 2D array.
//! \nick
//! \version $Rev: 5 $
//! \tparam dataT Template parameter for type of data
//! \tparam nrow Template parameter for number of rows
//! \tparam nnz Template paramter for number of non-zeros
//****************************************************************************80
#include "my_incl.h"
...
//****************************************************************************80
  friend std::ostream& operator << (std::ostream& os, 
                                    const StaticList2D<dataT, nrow, nnz>& a)
  {
...
//****************************************************************************80
  inline int* get_index_ptr(const intT& i )
  {
    ...
    return const_cast<int*>(static_cast<const StaticList2D&>(*this).get_index_ptr(i));
  }
...
private:
  ...
  intT index[nrow + 1]; /*!< Index pointer that allows for addressing 2D */
  dataT data[nnz]; /*!< Data pointer */
  double mem; /*!< Amount of memory in megabytes specified for the array */
//****************************************************************************80
//!
//! \brief StaticList2D : Default constructor 
//! \details
//! \nick
//! \version $Rev: 5 $
//!
...
//! \version $Rev: 5 $
//****************************************************************************80
  StaticList2D(const StaticList2D<dataT, nrow, nnz >&) = delete; //blocked
...
//! \version $Rev: 5 $
//****************************************************************************80
  StaticList2D& operator = (const StaticList2D<dataT, nrow, nnz>&) = delete; 
// Blocked
