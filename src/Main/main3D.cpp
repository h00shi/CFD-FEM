...
void print_main()
{
  ...
  std::cout << " +    ++++ +    +    +      +++  ++++ ++++ " << std::endl;
  std::cout << std::endl;
  std::cout << "            FEM_SOL: Solver for FEM Code 3D " 
	    << std::endl;
...
int main(int argc, char** argv)
{
  ...
  //---> Read mesh
  if(SolGlobalVars::MeshType=="fv-uns"){
    mesh_reader.read_fvuns_ascii(SolGlobalVars::Project, grid); 
  }
  ...
  if( SolGlobalVars::EquationType.compare("Euler") == 0) {
    ...
    uinf = SolGlobalVars::Fmach*cos(-pi/180.0*SolGlobalVars::Alpha) * 
      cos(pi/180.0*SolGlobalVars::Beta);
    vinf = SolGlobalVars::Fmach*sin(pi/180.0*SolGlobalVars::Beta);
    winf = SolGlobalVars::Fmach*sin(pi/180.0*SolGlobalVars::Alpha) * 
      cos(pi/180.0*SolGlobalVars::Beta);
    ...
    Solver<int,double, EulerPDESet3D<int,double> > solver(Euler, grid, soln);
