...
TEST(SurrealPow, DoubleRaisedToSurreal) {
  ...
  //test derivatives
  EXPECT_DOUBLE_EQ(std::pow(1.1,3.0) * std::log(x) * 3.0, c.Deriv(0));
  EXPECT_DOUBLE_EQ(std::pow(1.1,3.0) * std::log(x) * 4.0, c.Deriv(1));
  EXPECT_DOUBLE_EQ(std::pow(1.1,3.0) * std::log(x) * 5.0, c.Deriv(2));
}
