...
TEST(SurrealAdd, SurrealAddSurreal) {
  ...
  c = a + b;
  //test value
  EXPECT_DOUBLE_EQ(5.0, c.GetValue());
  //test derivatives
  EXPECT_DOUBLE_EQ(5.0, c.GetDeriv(0));
  EXPECT_DOUBLE_EQ(7.0, c.GetDeriv(1));
  EXPECT_DOUBLE_EQ(9.0, c.GetDeriv(2));
}
...
TEST(SurrealAdd, DoublePlusSurreal) {
  ...
  c = x + b;
  //test value
  EXPECT_DOUBLE_EQ(4.1, c.GetValue());
  //test derivatives
  EXPECT_DOUBLE_EQ(3.0, c.GetDeriv(0));
  EXPECT_DOUBLE_EQ(4.0, c.GetDeriv(1));
  EXPECT_DOUBLE_EQ(5.0, c.GetDeriv(2));
}
...
TEST(SurrealAdd, SurrealPlusDouble) {
  ...
  c = a + x;
  //test value
  EXPECT_DOUBLE_EQ(3.1, c.GetValue());
  //test derivatives
